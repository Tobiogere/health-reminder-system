...
const SearchPatient = () => {
  ...
  const handleSearch = async (e) => {
    ...
    if (!query) { setError('Please enter a patient ID to search.'); return; }
    try {
      setSearching(true);
      ...
      else setError(data.message || 'No patient found with that ID.');
    } catch (err) {
  ...
  const handlePrescriptionSubmit = async (prescriptionData) => {
    try {
      ...
      if (res.ok) {
        // Step 2 — Suggest custom drugs to admin if checkbox checked
        if (prescriptionData.customDrugs?.length > 0) {
          for (const drugName of prescriptionData.customDrugs) {
  ...
  return (
    <PageWrapper sidebarOpen={sidebarOpen} onToggleSidebar={() => setSidebarOpen(!sidebarOpen)} onCloseSidebar={() => setSidebarOpen(false)}>
      ...
      <div className="mb-4">
        <h2 style={{ fontWeight: 700, fontSize: '1.3rem', marginBottom: '0.2rem' }}>🔍 Search Patient</h2>
        <p style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>Search by matric number, staff ID or phone number</p>
      </div>
      ...
      <div className="card-custom mb-4">
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <input type="text" className="form-control"
              ...
              onChange={e => {
                ...
                setError('');
                if (!e.target.value) { setSearchResult(null); setSearched(false); }
              }}
        ...
        {error && <div className="alert alert-danger py-2 mt-3 mb-0" style={{ fontSize: '0.82rem' }}>{error}</div>}
      </div>

      {searched && searchResult && (
        <div className="card-custom">
          <h5 style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '1rem' }}>Patient Found</h5>
          <div style={{ padding: '1rem', borderRadius: '8px', border: '1px solid #e9ecef', backgroundColor: '#fff' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.75rem' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.4rem', flexWrap: 'wrap' }}>
                  <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>{searchResult.name}</span>
      ...
      {!searched && (
        <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--muted)' }}>
          ...
          <div style={{ fontWeight: 600, fontSize: '0.95rem', marginBottom: '0.3rem' }}>Search for a Patient</div>
          <div style={{ fontSize: '0.82rem' }}>Enter a matric number, staff ID or phone number above to get started</div>
        </div>
      )}
